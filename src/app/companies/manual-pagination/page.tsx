...
export default function CompaniesManualPaginationPage() {
  ...
  // Use search query when there's a search term, otherwise get all companies
  ...
  const allCompaniesResults = useQuery(
    api.companies.listPaginatedWithSearch,
    !debouncedSearchTerm.trim()
      ? {
          search: '',
  ...
  const results = debouncedSearchTerm.trim() ? searchResults : allCompaniesResults
  ...
  const toggleSortOrder = () => {
    ...
    setCursor(null) // Reset pagination when sorting changes
  }
  ...
  const handlePageSizeChange = (value: string) => {
    ...
    setCursor(null) // Reset pagination when page size changes
  }
      ...
      <div className="bg-white dark:bg-gray-800 shadow-lg rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-center justify-between border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 px-5 py-4">
          <div className="text-sm text-gray-600 dark:text-gray-400">
            Showing {results?.page.length || 0} companies
            {results?.isDone === false && ' (more available)'}
          </div>
