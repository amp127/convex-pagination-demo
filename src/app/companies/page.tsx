...
export default function CompaniesPage() {
  ...
  const openModal = useCallback(
    (mode: 'add' | 'edit' | 'view', company?: Doc<'companies'>, companyId?: Id<'companies'>) => {
      setModalState({
        isOpen: true,
        mode,
        company,
  ...
  // Fake data generator
  const generateFakeCompanies = useCallback((count: number = 20) => {
  ...
  const updateDatabaseSearch = useMemo(
    () =>
      debounce((value: string) => {
        ...
        companies.args.search = value
      }, 250),
    [companies.args] // ! args must be included or this breaks with just companies 
  )
  ...
  const columns: ColumnDef<Doc<'companies'>>[] = useMemo(
          ...
          return (
            <div>
              <Button
                ...
                onClick={() => openModal('view', row.original, row.original._id as Id<'companies'>)}
              >
    ...
    [openModal, order, companies, handleDeleteCompany]
  )
